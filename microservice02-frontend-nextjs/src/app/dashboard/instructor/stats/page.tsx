...
export default function InstructorGradeStatsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCourses() {
      ...
      try {
        ...
        if (Array.isArray(data)) {          const validatedCourses = data.map((course, idx) => ({
      ...
      <div className="flex justify-between items-center bg-white/60 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
        <div>
          <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-800 to-indigo-800 bg-clip-text text-transparent">
            Available Course Statistics
          </h1>
          <p className="text-sm text-gray-800 mt-1">
            Track available Course Statistics
          </p>
        </div>
      </div>
      {error && (
                ...
                <tbody className="divide-y divide-gray-200">
                  {courses.map(course => (
                    ...
                    >
                      ...
                      <td className="p-2 border text-gray-900">{course.initialDate ? new Date(course.initialDate).toLocaleDateString() : '-'}</td>
                      <td className="p-2 border text-gray-900">{course.finalDate ? new Date(course.finalDate).toLocaleDateString() : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
